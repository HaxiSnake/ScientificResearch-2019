...
class ProFundSummaryForm(ModelForm):
	class Meta:
		...
		exclude = ('content_id','project_id','finance_comment','equcosts_budget','equcosts_expenditure','finance_account','finance_staff','finance_checktime','serial_number',"remarkment",'conference_remark','cooperation_remark')
...
class ProFundSummaryRemarkmentForm(ModelForm):
        class Meta:
                model = ProjectFundSummary
                fields = ["remarkment"]
                exclude = ('conference_remark','cooperation_remark')
                widgets ={
                        "remarkment":forms.Textarea(attrs={"rows":"40","cols":"100"}),
                }
...
class ProFundBudgetForm(ModelForm):
	class Meta:
		...
		exclude = ('content_id','project_id','finance_comment','equcosts_budget','finance_staff','finance_checktime','serial_number',"remarkment",'conference_remark','cooperation_remark')
                widgets ={
                        #"remarkment":forms.Textarea(attrs={"rows":"40","cols":"100"}),
                        "finance_staff":forms.TextInput(attrs={"readonly":"readonly"}),
                        "finance_checktime":forms.TextInput(attrs={"readonly":"readonly"}),
...
class ProjectCreationForm(forms.Form):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields["special"].choices = tuple((special.id, special.name) for special in Special.objects.filter(application_status = True))
...
class ProjectCreationTeacherForm(ProjectCreationForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields["teacher"].choices = tuple((teacher.userid.id, "%s(%s)"%(teacher.userid.first_name,teacher.college)) for teacher in TeacherProfile.objects.all() )
        self.fields["special"].choices = tuple((special.id, special.name) for special in Special.objects.all())
