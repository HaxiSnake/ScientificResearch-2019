...
class ProjectJudgeForm(forms.Form):
    ...
    noPassReason = forms.MultipleChoiceField(required=False,
    ...
    skip_judge = forms.ChoiceField(required=False,widget=forms.RadioSelect(attrs={'name':'skip_choice'}))

    def __init__(self, *args,**kwargs):
        try:
            status=int(kwargs.get("status",None))
        except:
            status=None
        if status != None :
            del kwargs['status']
        super(ProjectJudgeForm, self).__init__(*args, **kwargs)
        if status == PROJECT_STATUS_APPLICATION_COLLEGE_OVER or status == PROJECT_STATUS_FINAL_COLLEGE_OVER:
            self.fields["skip_judge"].choices = ((True,"不需要评审"),(False,"需要评审"))
        if status in (PROJECT_STATUS_APPLICATION_COLLEGE_OVER, PROJECT_STATUS_APPLICATION_COMMIT_OVER):
            self.fields["noPassReason"].choices = ((u"网上申请不合格",u"网上申请不合格"),(u"申报书不合格",u"申报书不合格"))
        elif status in (PROJECT_STATUS_FINAL_COMMIT_OVER,PROJECT_STATUS_FINAL_COLLEGE_OVER,):
            self.fields["noPassReason"].choices = ((u"网上提交不合格",u"网上提交不合格"),(u"结题书不合格",u"结题书不合格"))
