...
def LookThroughResult(request,judgeid,userrole,userstatus,page,page2,search,look_through_form,searchForm,skip_judge=False):
    ...
    if form["judgeresult"]=="1":
        ...
        choices_noPassReason=form.getlist('noPassReason')

        if identity == FINANCE_USER or len(choices_noPassReason)!=1 :
            statusRollBack(request,project,3)
            print "11"
        elif u"网上申请不合格" in choices_noPassReason or u"网上提交不合格" in choices_noPassReason:
